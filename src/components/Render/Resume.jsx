...
const ResumeContainer = styled.div`
  padding: ${({ type }) => (type === "render" ? "1.5rem 1.5rem 2rem 1.5rem" : "0 0 0 1.5rem")};
  color: white;
  width: 100%;
...
const DetailsSection = ({ title, items, type, handleCancelClick }) => {
  return (
    <div>
      {/* CATEGORY TITLE */}
      ...
      {items &&
        ...
        items.map((item, index) => (
          <LineInformation key={index} render={type === "render"}>
            {type === "render" && (
              <CancelIcon src="cancel.svg" alt="Icône de suppression" onClick={() => handleCancelClick(item)} />
            )}
            <FacadeTitle>
              <p>-</p>
              {item.name}
...
export default function Resume({ renderRef, type, configuration, itemIndex }) {
  ...
  // DEPEND SI LA CONFIGURATION PROVIENT DU RENDER (DONC CHOICES DU CONTEXT) OU DU PANIER (DONC CONFIGURATION DU LOCAL STORAGE)
  const currentConfig = type === "render" ? choices : configuration;
  ...
  // --- SHOW MODAL FOR SUPPRESSION ---

  const handleCancelClick = (itemId) => {
    setResumeItem(itemId);
    setShowModal(true);
  };
  ...
  return (
    <>
      <ResumeContainer type={type}>
        {type === "render" && <TitleStyle color="white">CONFIGURATION</TitleStyle>}
        ...
        <DetailsSection
          ...
          type={type}
          handleCancelClick={handleCancelClick}
          items={getAllItems(currentConfig, "prises")}
        />
        ...
        <DetailsSection
          ...
          type={type}
          handleCancelClick={handleCancelClick}
          items={getAllItems(currentConfig, "gravures")}
        />
      ...
      {showModal && <Modal type="render" resumeItem={resumeItem} closeModal={closeModal} />}
    </>
  );
}
